### Running Code for the Global Coastal Permeability Database (GCPdb)
### code documentation in addition to
#### Result, Dataset Publication: Tschaikowski, Jarrid; Jenkins, Chris; Reinecke, Robert; Nozik, Alexandra; Moosdorf, Nils: Global Coastal Permeability database (GCPdb). PANGAEA, https://doi.pangaea.de/10.1594/PANGAEA.958901
### Language requirements: R

###############################

### Dataset check before import
### for documentation of dataset preparation see Technical appendix of data publication

### Dataset should include columns named
# MasterKey: necessary for join with ECU data by Sayre et al 
# CONTINENT
# Glim_Lithology
# Glim_unconsolidated_Occ
# Glim_consolidated_Occ
# Glhymps_Ferr_Log100
# Glympse_K_std_devX100
# Sayre_Waveheight
# Young_Cliff_Percantage
# Sayre_Max_Slope
# Young_Sayre_Slope_Occurence
# Luijendijk_Beach_Occ
# Artificial_Occurrence
# Spaldin_Mangrove_Occ
# Mcowen_Saltmarsh_Occ
# Spalding_Coral_Occ
# UNEP_Seagrass_Occ
# Lehner_river_Occ_500m_Buffer
# Lehner_river_Occ_2000m_Buffer
# Lehner_Estuary_Occ_2000m_Buffer
# Lehner_Estuary_Occ_5000m_Buffer
# dbS_Gravel_Percentage
# dbS_Sand_Percentage
# dbS_Mud_Percentage
      
###############################

##Import dataset in R for calculation
##GCPM<-read.csv("PATH GCPdb.csv)
	   
### START CALCULATION

###A-Classes#######################################
### Landward permeability 
...
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="B1",-1052,NA)
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="B2",-1052,GCPM$B_Perm_log100_max)
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="B3",-1052,GCPM$B_Perm_log100_max)
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="P",-2000,GCPM$B_Perm_log100_max)
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="M1",-1200,GCPM$B_Perm_log100_max)
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="M2",-1100,GCPM$B_Perm_log100_max)
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="R",GCPM$Glhymps_Ferr_Log100+GCPM$Glympse_K_std_devX100,GCPM$B_Perm_log100_max) # correct value input
GCPM$B_Perm_log100_max<-ifelse(GCPM$B_Perm_class=="C",-933,GCPM$B_Perm_log100_max)
...
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="B1",-1100,NA)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="B2",-1100,GCPM$B_Perm_log100_min)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="B3",-1100,GCPM$B_Perm_log100_min)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="P",-2000,GCPM$B_Perm_log100_min)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="M1",-1300,GCPM$B_Perm_log100_min)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="M2",-1200,GCPM$B_Perm_log100_min)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="R",GCPM$Glhymps_Ferr_Log100-GCPM$Glympse_K_std_devX100,GCPM$B_Perm_log100_max)
GCPM$B_Perm_log100_min<-ifelse(GCPM$B_Perm_class=="C",-1054,GCPM$B_Perm_log100_min)
...
#C-Block 5 Compute the continental averages for C_log100
...
GCPM$C_Perm_log100_avg_min<-GCPM$C_Perm_log100_min
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="Asia",C_Asia_mean_min,GCPM$C_Perm_log100_avg_min)
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="Africa",C_Africa_mean_min,GCPM$C_Perm_log100_avg_min)
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="Australia",C_Australia_mean_min,GCPM$C_Perm_log100_avg_min)
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="Europe",C_Europe_mean_min,GCPM$C_Perm_log100_avg_min)
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="North America",C_NA_mean_min,GCPM$C_Perm_log100_avg_min)
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="South America",C_SA_mean_min,GCPM$C_Perm_log100_avg_min)
GCPM$C_Perm_log100_avg_min<-ifelse(is.na(GCPM$C_Perm_log100_avg_min) & GCPM$CONTINENT=="Oceania",C_Oceania_mean_min,GCPM$C_Perm_log100_avg_min)

#Export dataset as csv
##write.csv(GCPM, "NEW PATH GCPdb.csv", sep=";")
